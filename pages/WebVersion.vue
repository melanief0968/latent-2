<template>
  <!-- <div v-touch:swipe.right="swipeHandler"> -->
    ...
    <!-- <div class="book-container slider"> -->
      <section class="message-book-container">
...
export default {
  ...
  methods: {
   ...
   getOtherUser() {
      ...
      const firstUser = Object.values(user).find((userID1) => {
        return userID1 !== otherUser;
      });
    ...
    getContactName() {
      let otherUser = this.getOtherUser().user2;
      const contactName = this.$getters.listenUser(otherUser).name;
      return contactName;
    },
    getContactName1() {
      let User1 = this.getOtherUser().user1;
      const contactName = this.$getters.listenUser(User1).name;
      return contactName;
    },
    getGender(userID){
    ...
    userInfos(){
      this.name1 = this.getContactName1()
      let userID = this.getOtherUser().user1
      this.name2 = this.getContactName()
      let otherUser= this.getOtherUser().user2
      let attr2 = this.$getters.listenUser(otherUser).attributs
      let attr1 = this.$getters.listenUser(userID).attributs
      let g1 = this.getGender(userID)
      let g2 = this.getGender(otherUser)
